...
//			EVENT_QUEUE_FLOWS : info.max_event_queue_flows;
	num_flows = info.max_event_queue_flows;
	...
	eventdev_gbl->max_workers = config.nb_event_ports;
			    ...
			    int num)
{
	...
	if (queue->s.type == ODP_QUEUE_TYPE_PLAIN) {
		...
		if (odp_unlikely(port_id >= eventdev_gbl->max_workers)) {
			ODP_ERR("Max %" PRIu8 " scheduled workers supported\n",
				eventdev_gbl->max_workers);
			return 0;
		}
